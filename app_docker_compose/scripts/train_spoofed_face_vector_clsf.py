...
class FakeFaceImageModel(pl.LightningModule):
    ...
    def __init__(self,  num_classes: int = 2, freeze_backbone: bool = False, **kwargs: Any):
        ...
        final_layer = ('classifier' if hasattr(self.model, 'classifier') else
                       'fc' if hasattr(self.model, 'fc') else None)
        if not final_layer:
            raise ValueError(
                f"Unrecognized final layer of feat ext {self.model.__name__}")

        n_inputs = getattr(getattr(self.model, final_layer), "in_features")
...
def train_faces_with_cnn(
        train_dir: str, test_dir: str,
        max_epochs: int = 100, devices: int = 4):
    """
    ...
    trainer = pl.Trainer(max_epochs=max_epochs, devices=devices, num_nodes=1,
                         log_every_n_steps=5,
...
def train_vectors_with_dnn(
        ...
        max_epochs: int = 150, devices: int = 4):
    """
    ...
    train_loader = DataLoader(
        train_dataset, num_workers=8, batch_size=64, shuffle=True)
...
def train_vectors_with_sklearn_clsf(
        x_train: np.ndarray, y_train: np.ndarray, x_test: np.ndarray, y_test: np.ndarray):
    """
...
if __name__ == "__main__":
    ...
    y_train_split = np.concatenate(
        [np.zeros(len(X_train_real)), np.ones(len(X_train_fake))])
    y_test_split = np.concatenate(
        [np.zeros(len(X_test_real)), np.ones(len(X_test_fake))])
