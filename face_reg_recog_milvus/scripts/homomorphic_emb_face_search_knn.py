...
class Server:
    ...
    def connec_to_client(self, client_public_context: dict):
        ...
        self.he_server.from_bytes_context(client_public_context["s_context"])
        self.he_server.from_bytes_public_key(client_public_context["s_public_key"])
        self.he_server.from_bytes_relin_key(client_public_context["s_relin_key"])
        self.he_server.from_bytes_rotate_key(client_public_context["s_rotate_key"])
...
if __name__ == "__main__":
    ...
    server = Server()
    client = Client({"scheme": "ckks", "n": 2**13, "scale": 2**30, "qi_sizes": [30] * 5})
    ...
    # decode vector dists
    dec_vec_dists = [
        client.he_client.decrypt(PyCtxt(pyfhel=client.he_client, bytestring=enc_vec))[0] for enc_vec in enc_vec_dists
    ]

    closest_idx = np.argmin(dec_vec_dists)
    assert closest_idx == QUERY_IDX

    # retrieve the vector with knn
    knn_vec_bytes = server.get_knn(dec_vec_dists, 1)
    knn_vec_enc = [PyCtxt(pyfhel=client.he_client, bytestring=byte_vec) for byte_vec in knn_vec_bytes]
    print(knn_vec_enc)
