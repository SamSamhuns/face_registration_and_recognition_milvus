...
def test_insert_person_mysql(test_mysql_connec, mock_person_data_dict):
    ...
    resp = insert_person_data_into_sql(test_mysql_connec, MYSQL_TEST_TABLE, mock_person_data_dict(TEST_PERSON_MYSQL_ID))
...
def test_select_person_mysql(test_mysql_connec, mock_person_data_dict):
    ...
    resp = select_person_data_from_sql_with_id(test_mysql_connec, MYSQL_TEST_TABLE, TEST_PERSON_MYSQL_ID)
    assert resp == {
        "status": "success",
...
def test_select_all_person_mysql(test_mysql_connec, mock_person_data_dict):
    """Selects test person data from MySQL database."""
    resp = select_all_person_data_from_sql(test_mysql_connec, MYSQL_TEST_TABLE)
    assert resp == {
        "status": "success",
...
def test_delete_person_mysql(test_mysql_connec):
    ...
    resp = delete_person_data_from_sql_with_id(test_mysql_connec, MYSQL_TEST_TABLE, TEST_PERSON_MYSQL_ID)
