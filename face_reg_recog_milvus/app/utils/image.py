...
def pad_resize_image(
    cv2_img: np.ndarray, new_size: tuple[int, int] = (640, 480), color: tuple[int, int, int] = (125, 125, 125)
) -> np.ndarray:
    """
...
def draw_bbox_on_image(
    ...
    for i, box in enumerate(boxes):
        ...
        xmin, ymin, xmax, ymax = (max(xmin, 0), max(ymin, 0), min(xmax, img_w), min(ymax, img_h))
        # draw bbox on image
        cv2.rectangle(
            cv2_img,
            (xmin, ymin),
            (xmax, ymax),
            (0, 0, 255),
            thickness=max(int((img_w + img_h) / 600), 1),
            lineType=cv2.LINE_AA,
        )

        # draw rect covering text
        t_size = cv2.getTextSize(label, 0, fontScale=tline / 3, thickness=1)[0]
        point2 = xmin + t_size[0] + 3, ymin - t_size[1] - 5
        color = (0, 0, 0)
        if text_bg_alpha == 0.0:
            cv2.rectangle(cv2_img, (xmin - 1, ymin), point2, color, cv2.FILLED, cv2.LINE_AA)
        else:
            ...
            cv2_img[y_min:y_max, x_min:x_max, 0] = cv2_img[y_min:y_max, x_min:x_max, 0] * alpha_reserve + b_c * (
                1 - alpha_reserve
            )
            cv2_img[y_min:y_max, x_min:x_max, 1] = cv2_img[y_min:y_max, x_min:x_max, 1] * alpha_reserve + g_c * (
                1 - alpha_reserve
            )
            cv2_img[y_min:y_max, x_min:x_max, 2] = cv2_img[y_min:y_max, x_min:x_max, 2] * alpha_reserve + r_c * (
                1 - alpha_reserve
            )
        # draw label text
        cv2.putText(
...
def plot_one_box(bbox, img, wscale=1, hscale=1, color=None, label=None, line_thickness=None) -> None:
    ...
    """
    tline = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    ...
    if label:
        ...
        cv2.putText(
            img, label, (coord1[0], coord1[1] - 2), 0, tline / 3, [225, 255, 255], thickness=tfont, lineType=cv2.LINE_AA
        )
